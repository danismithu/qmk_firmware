...
uint32_t layer_state_set_user(uint32_t state) {
    ...
    default:
        ...
        if(isLeftHand) {                        // this sets the underside LEDs (1-6) to cyan on both sides and does nothing with the rest.
          for(int i=0; i < 27; i ++) {           // this also means that if I manually change LED color to red, the bottom 6 LEDS will ALWAYS be cyan.
            rgblight_setrgb_at(255, 0, 0, i); // it's the only way I found to make it switch back to underglow LEDs after I've used another layer.
          }
          for(int j=0; j < 27; j ++) {
